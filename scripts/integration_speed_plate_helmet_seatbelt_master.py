...
# FINAL V5 + SEATBELT + HELMET VOTE FIX (stable across test3 + test6)
#
# Adds (helmet FIX):
# - Normalize helmet labels (helmet/nohelmet) -> (HELMET_OK/NO_HELMET)
# - Decontaminate helmet votes inside entity (ignore weak tracks if a strong one exists)
# - Fallback to best track decision if entity result is UNKNOWN or too few valid votes
#
# Seatbelt logic unchanged.
...
VERSION = "INTEGRATION_FINAL_V5_TEST3_TEST6_WITH_SEATBELT_HELMETFIX"
DEBUG = True  # True أول مرة، بعدين False
...
# ================= Helmet vote fix (CRITICAL) =================
def _normalize_helmet_label(label: str) -> str:
    """
    Convert model outputs to canonical labels:
    - helmet -> HELMET_OK
    - nohelmet -> NO_HELMET
    ...
    if s in ("helmet", "withhelmet", "helmetok"):
        return "HELMET_OK"
    if s in ("nohelmet", "withouthelmet"):
        return "NO_HELMET"
    if "nohelmet" in s or ("no" in s and "helmet" in s):
        return "NO_HELMET"
    if "helmet" in s:
        return "HELMET_OK"
    return "UNKNOWN"

def _track_votes_total(helmet_by_track, tid):
...
def _best_track_for_helmet(ent_tracks, helmet_by_track):
    best_tid = None
    best_total = -1
    best_valid = -1

    for tid in ent_tracks:
        r = helmet_by_track.get(str(tid))
        if not r:
            continue
        v = r.get("votes", {}) or {}
        tot = int(v.get("total", 0) or 0)
        valid = int(v.get("with", 0) or 0) + int(v.get("without", 0) or 0)
        if tot > best_total or (tot == best_total and valid > best_valid):
            best_total = tot
            best_valid = valid
            best_tid = str(tid)
...
def _ensure_helmet_track_schema(helmet_by_track):
    """
    Your helmet_per_moto.json might have:
      final_decision: "helmet" / "nohelmet"
    We normalize in-place to HELMET_OK / NO_HELMET and keep votes as-is.
    """
    for tid, r in helmet_by_track.items():
        if not isinstance(r, dict):
            continue
        fd = r.get("final_decision", None)
        r["final_decision_norm"] = _normalize_helmet_label(fd)

def aggregate_helmet_votes_entity_decontam(ent_tracks, helmet_by_track, min_track_total=4):
    """
    Decontaminate votes:
    - If entity contains a strong track => ignore tracks with total < min_track_total
    - Else use all tracks (avoid too many UNKNOWN)
    """
    per = []
    for tid in ent_tracks:
        r = helmet_by_track.get(str(tid))
        if not r:
            per.append((str(tid), 0, None))
            continue
        v = r.get("votes", {}) or {}
        tot = int(v.get("total", 0) or 0)
        per.append((str(tid), tot, v))

    has_strong = any(tot >= min_track_total for _, tot, _ in per)

    votes = {"with": 0, "without": 0, "unknown": 0, "total": 0}
    used = 0
    skipped = 0

    for tid, tot, v in per:
        if v is None:
            continue
        if has_strong and tot < min_track_total:
            skipped += 1
            continue
        votes["with"] += int(v.get("with", 0) or 0)
        votes["without"] += int(v.get("without", 0) or 0)
...
def split_moto_tracks_into_entities(master_id, moto_tracks, samples, per_track, helmet_by_track):
    ...
    for i in range(len(tids)):
        for j in range(i + 1, len(tids)):
            ...
            htA = _track_votes_total(helmet_by_track, A["tid"])
            htB = _track_votes_total(helmet_by_track, B["tid"])
            ...
            d_med = dist(A["med_c"], B["med_c"]) if (A["med_c"] and B["med_c"]) else 1e18

            if (weakA or weakB) and ov_ratio >= 0.60 and d_med <= DUP_CENTER_WEAK_PX:
                dsu.union(A["tid"], B["tid"])
                continue

    # 2) stitch sequential splits mutual-best (unchanged logic)
    best_succ = {}
    best_prev = {}

    def weakish_for_stitch(st):
        ht = _track_votes_total(helmet_by_track, st["tid"])
    ...
    def consider_link(A, B):
        ...
        # we do not have bbox here in this trimmed stats (only med_c), use med_c as proxy
        d_med = dist(A["med_c"], B["med_c"]) if (A["med_c"] and B["med_c"]) else 1e18

        ok_norm = (gap <= STITCH_GAP_SEC and d_med <= STITCH_MED_CENTER_MAX_PX)
        ok_ext = False
        if len(tids) == 2 or weakish_for_stitch(A) or weakish_for_stitch(B):
            ok_ext = (gap <= STITCH_GAP_SEC_EXT and d_med <= STITCH_MED_CENTER_MAX_PX_EXT)

        if not (ok_norm or ok_ext):
            return

        sc = (gap / (STITCH_GAP_SEC if ok_norm else STITCH_GAP_SEC_EXT)) + (d_med / (STITCH_MED_CENTER_MAX_PX if ok_norm else STITCH_MED_CENTER_MAX_PX_EXT))
        a = A["tid"]; b = B["tid"]
    ...
    entities.sort(key=lambda e: min([stats.get(x, {}).get("start_t", 1e18) for x in e]) if e else 1e18)
...
def merge_spurious_entities_post(master_id, entities, per_track, helmet_by_track, stats_map):
    # keep your existing logic exactly (as you pasted) — unchanged
    # (I’m not rewriting it here to avoid accidental divergence)
    # --- START: original function content (unchanged) ---
    ...
    def ent_meta(ent):
        ...
        for tid in ent:
            ...
            ht_sum += _track_votes_total(helmet_by_track, tid)
...
def main():
    ...
    _ensure_helmet_track_schema(helmet_by_track)
    ...
    for mid in sorted(masters.keys(), key=lambda x: (0 if x.startswith("M") else 1, x)):
        ...
        for idx, ent_tracks in enumerate(entities, start=1):
            ...
            # ✅ decontaminate votes
            votes, used_tracks, skipped = aggregate_helmet_votes_entity_decontam(ent_tracks, helmet_by_track, min_track_total=4)
            ...
            # ✅ fallback to best track if still weak/unknown
            valid = int(votes["with"]) + int(votes["without"])
            if (final == "UNKNOWN") or (valid < MIN_VALID_VOTES):
                best_tid = _best_track_for_helmet(ent_tracks, helmet_by_track)
                if best_tid is not None:
                    br = helmet_by_track.get(best_tid, {})
                    fd = br.get("final_decision_norm", "UNKNOWN")
                    if fd in ("HELMET_OK", "NO_HELMET"):
                        final = fd
                        viol = (final == "NO_HELMET")
                        rule = f"fallback_best_track={best_tid}"
                        if DEBUG:
                            print(f"[FIX] {ent_mid}: fallback -> best_track {best_tid} => {final}")
    ...
    print("\n=== FINAL REPORT (MASTER + SPEED + PLATE + HELMET + SEATBELT) ===")
    print(f"Cars: {len(cars_out)} | Motorcycles: {len(motos_out)}")
    for c in cars_out:
        print(f"- CAR id={c['car_id']} master={c['master_id']} speed={c['speed']['max_stable_kmh']} plate={c['plate']['value']} seatbelt={c['seatbelt']['final_decision']}")
    for m in motos_out:
