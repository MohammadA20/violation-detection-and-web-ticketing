# scripts/integration_speed_plate_helmet_seatbelt_master.py
# FINAL V5 + SEATBELT + PHONE (stable across test3 + test6)
#
# This is a NEW integration script. Your old integration stays untouched.
#
# Adds:
# - Load results/seatbelt_per_car.json
# - Load results/phone_per_car.json  ✅ NEW
# - Aggregate seatbelt votes per CAR master (across tracks)
# - Aggregate phone votes per CAR master (across tracks)
# - Ignore seatbelt/phone for MOTORCYCLE outputs
...
VERSION = "INTEGRATION_FINAL_V5_TEST3_TEST6_WITH_SEATBELT_AND_PHONE"
DEBUG = True  # True أول مرة، بعدين فيك False
...
# ✅ Seatbelt
SEATBELT_PER_CAR = Path("results/seatbelt_per_car.json")

# ✅ Phone (with robust fallback names)
PHONE_PER_CAR_CANDIDATES = [
    Path("results/phone_per_car.json"),
    Path("results/phone_per_car_from_speed.json"),
    Path("results/phone_speedlink.json"),
    Path("results/phone_per_car_speedlink.json"),
]

# output (keep same name for compatibility)
OUT_JSON         = Path("results/final_report.json")

# ---------------- Helmet/Seatbelt/Phone decision ----------------
MIN_VALID_VOTES = 1
RATIO_DECIDE = 0.55
...
def pick_first_existing(paths, default_path=None):
    for p in paths:
        try:
...
def decide_seatbelt(with_cnt: int, without_cnt: int):
    valid = int(with_cnt) + int(without_cnt)
    if valid < MIN_VALID_VOTES:
        return ("UNKNOWN", False, "few_valid")
    ...
    if r_without >= RATIO_DECIDE:
        return ("NO_SEATBELT", True, "ratio>=thr")
    if r_with >= RATIO_DECIDE:
        return ("SEATBELT_OK", False, "ratio>=thr")

    margin = abs(without_cnt - with_cnt)
    if valid >= FALLBACK_MIN_VALID and margin >= FALLBACK_MIN_MARGIN:
        if without_cnt > with_cnt:
            return ("NO_SEATBELT", True, "fallback_majority")
        if with_cnt > without_cnt:
            return ("SEATBELT_OK", False, "fallback_majority")

    return ("UNKNOWN", False, "uncertain")

# ✅ NEW: phone decision
# Standard meaning we enforce internally:
#   - votes["with"]    = NO_PHONE (OK)
#   - votes["without"] = PHONE (VIOLATION)
def decide_phone(with_cnt: int, without_cnt: int):
    valid = int(with_cnt) + int(without_cnt)
    if valid < MIN_VALID_VOTES:
        return ("UNKNOWN", False, "few_valid")
    ...
    if r_without >= RATIO_DECIDE:
        return ("PHONE", True, "ratio>=thr")
    if r_with >= RATIO_DECIDE:
        return ("NO_PHONE", False, "ratio>=thr")

    margin = abs(without_cnt - with_cnt)
    if valid >= FALLBACK_MIN_VALID and margin >= FALLBACK_MIN_MARGIN:
        if without_cnt > with_cnt:
            return ("PHONE", True, "fallback_majority")
        if with_cnt > without_cnt:
            return ("NO_PHONE", False, "fallback_majority")
...
# ================= Seatbelt helpers =================
def index_seatbelt_by_track(lst):
    d = {}
    if isinstance(lst, list):
        for r in lst:
            tid = r.get("track_id", None)
            if tid is None:
                continue
            d[str(norm_tid(tid))] = r
    return d

def aggregate_seatbelt_votes(tracks, seatbelt_by_track):
    votes = {"with": 0, "without": 0, "unknown": 0, "total": 0}
    used = 0
    for tid in tracks:
        r = seatbelt_by_track.get(str(tid))
        if not r:
            continue
        v = r.get("votes", {}) or {}
...
def _is_phone_decision(dec: str):
    dec = (dec or "").strip().upper()
    return ("PHONE" in dec) and ("NO_PHONE" not in dec) and ("NOPHONE" not in dec)

def _is_no_phone_decision(dec: str):
    dec = (dec or "").strip().upper()
    return ("NO_PHONE" in dec) or ("NOPHONE" in dec) or (dec == "SAFE_NO_PHONE")

def infer_phone_votes_orientation(phone_list):
    """
    Detect if phone votes are stored like:
      A) with=NO_PHONE, without=PHONE  (default/standard)
      B) with=PHONE, without=NO_PHONE  (reversed)
    Returns: True if standard (A), False if reversed (B)
    """
    ...
    for r in phone_list:
        v = (r.get("votes") or {})
        w  = int(v.get("with", 0) or 0)
        wo = int(v.get("without", 0) or 0)
        dec = str(r.get("final_decision", "") or "")

        if _is_phone_decision(dec):
            # if decision=PHONE, likely violation side count should dominate
            if wo > w:
                score_standard += 1   # standard means "without"=PHONE
            elif w > wo:
                score_reversed += 1
        elif _is_no_phone_decision(dec):
            # if decision=NO_PHONE, ok side count should dominate
            if w > wo:
                score_standard += 1
            elif wo > w:
                score_reversed += 1
    ...
    if DEBUG:
        print(f"[INFO] Phone vote-orientation hint: standard={score_standard} reversed={score_reversed}")

    if score_standard == 0 and score_reversed == 0:
        return True
    return (score_standard >= score_reversed)

def aggregate_phone_votes(tracks, phone_by_track, phone_standard=True):
    """
    returns votes in STANDARD meaning always:
      votes["with"]    = NO_PHONE
      votes["without"] = PHONE
    """
    raw = {"with": 0, "without": 0, "unknown": 0, "total": 0}
    used = 0
    for tid in tracks:
        r = phone_by_track.get(str(tid))
        if not r:
            continue
        v = r.get("votes", {}) or {}
        raw["with"] += int(v.get("with", 0) or 0)
        raw["without"] += int(v.get("without", 0) or 0)
        raw["unknown"] += int(v.get("unknown", 0) or 0)
        raw["total"] += int(v.get("total", 0) or 0)
        used += 1
    ...
    # reversed -> swap
    fixed = {
        "with": raw["without"],      # becomes NO_PHONE
        "without": raw["with"],      # becomes PHONE
        "unknown": raw["unknown"],
        "total": raw["total"],
    }
    return fixed, used
...
def salvage_missing_motos_from_samples(speed, per_track):
    ...
    return added
# ================= evidance =================
def pick_evidence_from_record(r, crop_dir):
    if not isinstance(r, dict):
        return None

    for k in ["best_evidence", "evidence_image", "image", "best_image"]:
        v = r.get(k)
        if v:
            return str(v).replace("\\", "/")

    for k in ["evidence_paths", "evidence", "crop_paths"]:
        v = r.get(k)
    ...
    tid = str(norm_tid(r.get("track_id")))
    if not tid:
        return None

    files = sorted(Path(crop_dir).glob(f"tid{tid}_*.jpg"))
    if not files:
        return None
...
def main():
    ...
    # ✅ seatbelt load
    seatbelt_by_track = index_seatbelt_by_track(load_json(SEATBELT_PER_CAR, []))
    seatbelt_evidence_by_track = {
         tid: pick_evidence_from_record(r, "results/seatbelt_crops")
         for tid, r in seatbelt_by_track.items()
    } 
    if DEBUG:
        print(f"[INFO] Seatbelt records loaded: {len(seatbelt_by_track)} from {SEATBELT_PER_CAR}")

    # ✅ phone load (robust)
    phone_path = pick_first_existing(PHONE_PER_CAR_CANDIDATES, None)
    phone_list = load_json(phone_path, []) if phone_path else []
    phone_by_track = index_phone_by_track(phone_list)
    phone_evidence_by_track = {
        
        tid: pick_evidence_from_record(r, "results/phone_crops")
        for tid, r in phone_by_track.items()
    }
    phone_standard = infer_phone_votes_orientation(phone_list)  # True means with=NO_PHONE, without=PHONE
    if DEBUG:
        print(f"[INFO] Phone records loaded: {len(phone_by_track)} from {phone_path}")
        print(f"[INFO] Phone vote orientation: {'STANDARD(with=NO_PHONE, without=PHONE)' if phone_standard else 'REVERSED(with=PHONE, without=NO_PHONE)'}")
    ...
    for mid in sorted(masters.keys(), key=lambda x: (0 if x.startswith("M") else 1, x)):
        ...
        if typ == "CAR":
            ...
            # ✅ seatbelt aggregation per car master
            sb_votes, sb_used = aggregate_seatbelt_votes(tracks, seatbelt_by_track)
            sb_final, sb_viol, sb_rule = decide_seatbelt(sb_votes["with"], sb_votes["without"])

            # ✅ phone aggregation per car master
            ph_votes, ph_used = aggregate_phone_votes(tracks, phone_by_track, phone_standard=phone_standard)
            ph_final, ph_viol, ph_rule = decide_phone(ph_votes["with"], ph_votes["without"])
            ...
            cars_out.append({
                ...
                "confidence": round(float(plate_conf), 3),
                "helmet": {
                    "final_decision": "ITS_CAR",
                    "violation": False
                },
                "seatbelt": {
                    "final_decision": sb_final,
                    "violation": bool(sb_viol),
                    "decision_rule": sb_rule,
                    "votes": sb_votes,
                    "tracks_used": int(sb_used)
                },
                "phone": {
                    "final_decision": ph_final,          # PHONE / NO_PHONE / UNKNOWN
                    "violation": bool(ph_viol),          # True if PHONE
                    "decision_rule": ph_rule,
                    "votes": ph_votes,                   # standardized: with=NO_PHONE, without=PHONE
                    "tracks_used": int(ph_used),
                    "source_file": str(phone_path) if phone_path else None
                },
                "evidence": {
                    "seatbelt_image": seatbelt_evidence_by_track.get(str(tracks[0])),
                    "phone_image": phone_evidence_by_track.get(str(tracks[0]))
                }
        ...
        for idx, ent_tracks in enumerate(entities, start=1):
            ...
            plate_val, plate_conf, plate_from = pick_plate_for_master(ent_tracks, plate_by_carid)

# fallback: if only one plate was detected in the video, assign it to the motorcycle
            if plate_val == "UNKNOWN" and len(plate_by_carid) == 1:
                only = list(plate_by_carid.values())[0]
                plate_val = str(only.get("final_plate") or "UNKNOWN").strip().upper()
                plate_conf = float(only.get("ocr_conf", 0.0) or 0.0)
                plate_from = str(only.get("car_id"))
            ...
            helmet_image = None
            for t in ent_tracks:
                hr = helmet_by_track.get(str(t))
                if not hr:
                    continue
                if helmet_confidence is None:
                    helmet_confidence = hr.get("confidence")
                ev = hr.get("evidence", {}) or {}
                if helmet_image is None:
                    helmet_image = ev.get("helmet_image")
            ...
            motos_out.append({
                ...
                # ✅ ignore seatbelt+phone for motos
                "seatbelt": {
                    "final_decision": "IGNORED_MOTO",
                    "violation": False
                },
                "phone": {
                    "final_decision": "IGNORED_MOTO",
    ...
    print("\n=== FINAL REPORT (MASTER + SPEED + PLATE + HELMET + SEATBELT + PHONE) ===")
    print(f"Cars: {len(cars_out)} | Motorcycles: {len(motos_out)}")
    for c in cars_out:
        print(f"- CAR id={c['car_id']} master={c['master_id']} speed={c['speed']['max_stable_kmh']} plate={c['plate']['value']} seatbelt={c['seatbelt']['final_decision']} phone={c['phone']['final_decision']}")
    for m in motos_out:
